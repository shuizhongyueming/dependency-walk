...
export const parserCjs: (requireFunctionNames?: string[]) => ModuleParser = (
  ...
  const isPlainRequire = (node: namedTypes.CallExpression) => {
    return (
  ...
  const isMainScopeRequire = (node: namedTypes.CallExpression) => {
    const { callee } = node;
    return (
  ...
  const extractDependencyFromRequire = (node: namedTypes.CallExpression) => {
  ...
  const extractDependencyFromMainRequire = (node: namedTypes.CallExpression) => {
    ...
    visit(ast, {
      visitCallExpression(nodePath) {
        ...
        if (isPlainRequire(node)) {
          ...
          return false;
        } else if (isMainScopeRequire(node)) {
